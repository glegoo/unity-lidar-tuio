...
namespace Assets.SerialPortUtility.Scripts
{
    public class ControlPanelAssignment : MonoBehaviour
    {
        ...
        public InputField ratioMMInput;
        public InputField ratioPXInput;
        ...
        void Awake()
        {
            ...
            if (PlayerPrefs.HasKey(ratioMM.ToString())) ratioMMInput.text = PlayerPrefs.GetFloat(ratioMM.ToString()).ToString();
            if (PlayerPrefs.HasKey(ratioPX.ToString())) ratioPXInput.text = PlayerPrefs.GetFloat(ratioPX.ToString()).ToString();
